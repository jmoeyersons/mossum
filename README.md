...
Mossum_local is a tool for summarizing results from Stanford's
...
```
pip3 install git+https://github.com/jmoeyersons/mossum@local
```
...
```
mossum_local MossURL1 MossURL2 ... MossURLn
```

If no URLs are specified as parameters, the URLs are read from the
standard input. In this example, mossum_local generates *n* PNG images, that
...
By default mossum_local only shows a link between two submissions if the match Moss
finds
...
```
mossum_local -p 95 -l 30 MossURL1 MossURL2 ... MossURLn
```

mossum_local only shows a link between submission if either match accounts for more
...
submissions in common. If the `--report` or `-r` flag is specified, mossum_local
...
```
mossum_local -r -t "*./(.+)/.*" MossURL1 MossURL2 ... MossURLn
```
...
The format of the image generated by mossum_local can be changed using the `--format`
or `-f` option. Mossum_local uses Graphviz to generate graphs. All output formats
supported by Graphviz are therefore supported by mossum_local. See [Graphviz output
...
```
mossum_local -f svg -t "*./(.+)/.*" -o assignment MossURL1 MossURL2 ... MossURLn
```
...
If you are using X, you can specify the format `xlib`. In this case mossum_local
